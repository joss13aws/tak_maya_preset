...
class XGenBaker(QDialog):
    ...
    def buildUI(self):
        ...
        mainLayout.addWidget(exportButton, 3, 1, 1, 4)
    ...
    def exportCollection(self):
        ...
        pm.mel.eval('arnoldExportAss -f "{0}" -boundingBox -compressed -shadowLinks 1 -lightLinks 1 -s -startFrame {1} -endFrame {2} -frameStep 1.0;'.format(self.savePathLineEdit.text(), float(self.startFrameLineEdit.text()), float(self.endFrameLineEdit.text())))