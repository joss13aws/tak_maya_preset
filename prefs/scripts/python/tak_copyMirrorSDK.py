	...
	def main(cls, *args):
		...
		for srcDrvr in srcDrvrs:
			for drvrAttr in drvrAttrList:
				animCrvNodes = cmds.listConnections('%s.%s' %(srcDrvr, drvrAttr), scn = True, d = True, s = False)
				...
				if animCrvNodes:
					for animNode in animCrvNodes:
						...
						print '%s.%s' %(trgDrvr, drvrAttr), drvrVals, '%s.%s' %(trgDrvn, drvnAttr), drvnVals
						...
						for i in xrange(len(drvrVals)):
							...
							print '%s.%s' %(trgDrvr, drvrAttr), drvrVal, '%s.%s' %(trgDrvn, drvnAttr), drvnVal
