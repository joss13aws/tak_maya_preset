...
class UI(object):
	...
	def ui(cls):
		...
		cmds.text(label = 'Image Plane: ')
		cls.widgets['imgPlaneTxtFldBtnGrp'] = cmds.textFieldButtonGrp(columnWidth = [1, 110], buttonLabel = '...', bc = Functions.getImgPath, text = 'P:/1602_E35/Misc/E35_safeFrame.png')
...
class Functions(object):
	...
	def setShotCam(*args):
		...
		# Set image plane
		imgPlanePath = cmds.textFieldButtonGrp(UI.widgets['imgPlaneTxtFldBtnGrp'], q = True, text = True)
		imgPlaneNode = cmds.shadingNode('imagePlane', asUtility = True)
		cmds.connectAttr(imgPlaneNode + '.message', camShp + '.imagePlane', f = True)
		cmds.setAttr(imgPlaneNode + '.imageName', imgPlanePath, type = 'string')
		cmds.setAttr(imgPlaneNode + '.fit', 0)
