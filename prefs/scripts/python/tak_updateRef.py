...
07/23/2014 v1.2
항상 업데이트 창이 팝업 되고, 모든 에셋을 바꿀 수 있도록 수정.
'''

import os, re
import maya.cmds as cmds
import tak_logging_b1
from functools import partial

# create logger for debugging
logger = tak_logging_b1.Logger()
...
def updateRef(filePath):   
    ...
    logger.debug('reference informations: ' + str(refInfos))
	...
	logger.info('### All references are latest version. ###')
	updateAssetUI(refInfos, contents, filePath)
...
def updateAssetUI(refInfos, contents, filePath):
    ...
    cmds.text(label = '업데이트 할 에셋을 체크하고 Update Selected Asset 버튼을 눌러 주세요.\n업데이트 하지 않으려면 Do not update 버튼을 누르세요.\n현재 씬을 유지 하려면 X 버튼을 눌러 창을 닫아 주세요.')
...
class AssetList:
	...
	logger.debug('release directory: ' + str(releaseDir))
	releaseFiles = os.listdir(releaseDir)
	logger.debug('release files: ' + str(releaseFiles))
...
def updateAsset(assetListObjs, contents, filePath, *args):
	...
	if chkBoxOpt:
	    ...
	    logger.debug('current reference path: %s' %(assetListObj.currentReferencePath))
	    ...
	    logger.debug('selected reference path: %s' %(selectedReferencePath))
    ...
    logger.debug('updated file path: %s' %(updatedFilePath))
    
    # save as updated file
    fW = open(updatedFilePath, 'w')
    fW.write(contents)
    fW.close()  
    logger.info('### Updated file is saved as %s. ###' %(updatedFilePath))
