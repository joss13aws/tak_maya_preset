...
class batchPB():
	...
	def ui(cls):
		...
		cmds.textFieldGrp('camNameConvTxtFldGrp', p = 'optColLo', label = 'Camera Naming Convention: ')
	...
	def batchPB(cls, *args):
		...
		for srcfile in srcFileLs:
			...
			cmds.playblast(fo = True, format = formatOpt, sound = shotSound, filename = pbFilePath, sequenceTime = 0, clearCache = True, viewer = False, showOrnaments = True, framePadding = 4, percent = scaleOpt, compression = encodingOpt, quality = qualityOpt, widthHeight = [sizeW, sizeH])
