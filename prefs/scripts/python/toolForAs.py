...
def UI():
	...
	cmds.button(label = 'Remove custom nodes from advnced skeleton "Sets"', c = rmvCustomRigNodesFromSets)
	cmds.text(label = 'Delete Custom Constraint Connected to Advanced Skeleton Rig')
...
def rmvCustomRigNodesFromSets(*args):
	'''
	Remove all nodes of 'extra_ctrl_grp' from advanced skeleton set.
	'''

	cmds.select('extra_ctrl_grp', r = True, hi = True)
	if cmds.objExists('facial_gui_grp'):
		cmds.select('facial_gui_grp', add = True, hi = True)
