...
class Functions(object):
    ...
    def connectOri(cls, srcObj, pntInfoNode, anchorGrp):
        ...
        if cmds.nodeType(pntInfoNode) == 'pointOnSurfaceInfo':  # In case nurbs surface
            ...
            # input1 vector is worldY
            cmds.setAttr('%s.input1X' % zVecNode, 0)
            cmds.setAttr('%s.input1Y' % zVecNode, 1)
            cmds.setAttr('%s.input1Z' % zVecNode, 0)
        ...
        if cmds.nodeType(pntInfoNode) == 'pointOnSurfaceInfo':
            ...
            cmds.setAttr('%s.in11' % matrix, 1)
            cmds.setAttr('%s.in12' % matrix, 0)
